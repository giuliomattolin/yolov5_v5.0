...
class LoadImagesWoLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        cache_path = (p if p.is_file() else Path(self.img_files[0]).parent).with_suffix('.cache')  # cached labels
        if cache_path.is_file():
            cache, exists = torch.load(cache_path), True  # load
            if cache['hash'] != get_hash(self.img_files) or 'version' not in cache:  # changed
                cache, exists = self.custom_cache(cache_path, prefix), False  # re-cache
        else:
            cache, exists = self.custom_cache(cache_path, prefix), False  # cache
    ...
    def custom_cache(self, path=Path('./labels.cache'), prefix=''):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def collate_fn(batch):
        ...
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, label
